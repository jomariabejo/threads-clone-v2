  ...
  return (
    <Flex wrap="wrap" gap={4} align="center" justify="space-between">
      <Text fontSize="sm" color="fg.subtle">
        <FormattedMessage
          id="admin.users.pagination.summary"
          values={{
      ...
      <HStack gap={2}>
        <Button appearance="subtle" isDisabled={currentPage === 0} onClick={() => { onPageChange(0); }}>
          <FormattedMessage id="admin.users.pagination.first" />
        </Button>
        ...
        <Pagination
          ...
          max={5}
          label={intl.formatMessage({ id: 'admin.users.pagination.navLabel' })}
        />

        <Button appearance="subtle" isDisabled={currentPage >= pageCount - 1} onClick={() => { onPageChange(pageCount - 1); }}>
          <FormattedMessage id="admin.users.pagination.last" />
        </Button>
      </HStack>
      ...
      <HStack gap={3}>
        <HStack gap={2}>
          <Text fontSize="sm" color="fg.subtle" whiteSpace="nowrap">
            <FormattedMessage id="admin.users.pagination.pageSize" />
          </Text>
          <Box minW="80px">
            <Select<PageSizeOption>
              inputId="admin-users-page-size"
              options={pageSizeOptions}
              value={pageSizeOptions.find(option => option.value === pageSize)}
              onChange={(option: PageSizeOption | null) => { if (option) onPageSizeChange(option.value); }}
              isSearchable={false}
              spacing="compact"
              aria-label={intl.formatMessage({ id: 'admin.users.pagination.pageSize' })}
            />
          </Box>
        </HStack>
        ...
        <HStack gap={2}>
          <Box minW="80px">
            <Textfield
              ...
              placeholder={intl.formatMessage({ id: 'admin.users.pagination.goToPage' })}
              aria-label={intl.formatMessage({ id: 'admin.users.pagination.goToPage' })}
              isCompact
            />
          </Box>
          <Button appearance="subtle" isDisabled={!goToPageValue} onClick={handleGoToPage}>
            <FormattedMessage id="admin.users.pagination.go" />
          </Button>
        </HStack>
      </HStack>
    </Flex>
